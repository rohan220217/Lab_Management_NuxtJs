  ...
  <v-container>
    ...
    <v-card>
      ...
      contain
        ...
        height="400px"
        :src="`${loadedItems.link}`"
      >
        <v-card-title>{{loadedItems.name}}</v-card-title>
      </v-img>

      <v-card-subtitle class="pb-0"><v-rating :value="loadedItems.rating" color="amber" dense half-increments readonly size="14"></v-rating>

        <div class="grey--text ml-4">{{loadedItems.rating}} (413)</div></v-card-subtitle>

      <v-card-text class="text--primary">
        <div>Number of stock : {{loadedItems.in_stock}}</div>

        <div> Description:
        {{loadedItems.description}}</div>
      </v-card-text>
